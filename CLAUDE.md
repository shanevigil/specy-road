...
- **Version numbers are human-owned** — Do not bump `project.version`, write a new `## [v…]` CHANGELOG heading, name a `WIP/improvements-*` / `chore/release-v*` branch, or create a git tag unless the user stated the exact number in this conversation. Stop, highlight published PyPI vs declared checkout vs latest tag (and an optional proposal), and wait. See [`AGENTS.md`](AGENTS.md) (Version numbers) and [`.cursor/rules/027-version-numbers-require-human.mdc`](.cursor/rules/027-version-numbers-require-human.mdc).
