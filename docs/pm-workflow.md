...
- **Status column and dependency-step bars** — The **outline Status** text and **Gantt bar colors** use the roadmap JSON **and** active **`roadmap/registry.yaml`** rows: if a task has a registered **feature branch** but the chunk file still says **Not Started**, the PM view shows **In Progress** (blue bar) so you see claimed work on the same auto-refresh cadence as the rest of the chart. **Complete**, **Blocked**, **Cancelled**, and **In Progress** in the file are not overridden. Changing status in the UI still writes the roadmap JSON. Hover the Status cell when it differs from the saved file for a short explanation; the task editor notes when the list and the file disagree.
