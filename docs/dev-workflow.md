...
If you reach an `agentic` task whose planning sheet leaves what to build open, or
whose `## References` cite a contract that does not resolve to a real file —
**stop**. Flag it to the PM. A missing contract is a planning gap, not something
...
## Reading the task's planning sheet

The prompt file generated by `do-next-available-task` inlines the node's planning
sheet and every ancestor's, so what you need is already in front of you. Read it
for:

| Look for | What it tells you |
| --- | --- |
| `## Intent` | Exactly what to build or change |
| `## References` | Which contracts to conform to — **read them**; the brief inlines each cited `shared/` file in full |
| `## Approach` | Inputs → outputs (API shape, file format, component props), and the rules that bind you |
| `## 6. Dependency context` (in the brief) | What must exist before you start, with each dependency's own `## Intent` inlined |

A well-written sheet also names an observable **success signal** and any
explicit **prohibitions**; if yours does not, the task is still implementable,
but say so when you finish.
