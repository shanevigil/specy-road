...
1. **`## Brief`** — The same **work-packet brief** the CLI would generate for that node (`specy-road brief`): metadata, ancestor chain, **inlined planning sheets** along the ancestor path and for this node, **full text of every top-level `shared/*.md` file**, dependencies, touch-zone guidance, and a short rollup reference. This block can be **large** on big programs.
