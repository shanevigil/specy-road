...
Install the CLI (**Python 3.11+**) and, for a new application repo, run **`specy-road init project`**—see **[Install](#install)** and [docs/install.md](docs/install.md) for extras and scaffolding. After that, everyday use boils down to:
...
**Optional extras** (`[gui-next]`, `[review]`), **`specy-road init project`**, `roadmap/git-workflow.yaml`, IDE stubs (`specyrd`), and bootstrap prompts: **[docs/install.md](docs/install.md)**.

## More on how to work with it

Typical path in an **application** repository after `specy-road init project`: roadmap and planning for direction and detail; validation, export, and briefs for engineering and agents; registry and branches for safe parallel work. Maintainers working on **this** toolkit follow [AGENTS.md](AGENTS.md) and [docs/toolkit-development.md](docs/toolkit-development.md).
...
7. **Optional IDE commands** — `specyrd` installs stubs that invoke the CLI; see [docs/install.md#optional-specyrd-ide-command-stubs](docs/install.md#optional-specyrd-ide-command-stubs).

## Where to read next

| Document | Purpose |
| -------- | ------- |
| [docs/install.md](docs/install.md) | PyPI install, optional extras, `init project`, `specyrd`, bootstrap prompts |
...
| [docs/toolkit-development.md](docs/toolkit-development.md) | Developing the `specy-road` package in this repo (maintainers) |
...
| [suggested_prompts/](suggested_prompts/) | Adoption prompts ([details in docs/install.md](docs/install.md#optional-bootstrap-prompts)) |
...
To participate in development of the **specy-road** toolkit itself, contact the repository owner.
