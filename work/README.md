...
**Closing a task:** By default, **`specy-road finish-this-task`** deletes the toolkit session files for that node after a successful validate/export/digest: `work/brief-<NODE_ID>.md`, `work/prompt-<NODE_ID>.md`, and `work/implementation-summary-<NODE_ID>.md` (if present). Tracked copies are removed and the deletion is staged in the same bookkeeping commit. Planning sheets under `planning/` are unchanged. Skip removal with **`specy-road finish-this-task --no-cleanup-work`** or set **`cleanup_work_artifacts_on_finish: false`** in `roadmap/git-workflow.yaml`.
