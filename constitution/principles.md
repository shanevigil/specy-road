...
- **Hierarchical roadmap YAML:** Split the graph across logically organized files under [`roadmap/`](../roadmap/) (manifest + chunks); keep each file readable. No chunk should exceed **~500 lines** unless that file holds **exactly one** task/sub-task node (smallest grain), e.g. a very long checklist — enforceable via validation. The threshold is configurable in [`../constraints/file-limits.yaml`](../constraints/file-limits.yaml).
