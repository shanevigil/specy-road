...
A collection of audit and review prompts for maintaining code quality, architecture compliance, and release readiness.
...
Audit scope:
1. Architecture & ownership rules (from `docs/`):
...
Purpose: Review recently changed code for quality, correctness, and adherence to project conventions. Scoped to what has changed — not a full codebase scan.
...
2. Environment variable and config handling
   ...
   - Check that no API key or secret value is passed to logging or printed.
...
Purpose: Gate check before cutting a release or promoting a branch. Run this before any version tag or release merge.
