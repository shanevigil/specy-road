...
- Copy the full `templates/project` tree so you include: `schemas/*.json`, `work/README.md`, `AGENTS.md`, and a **minimal valid** `roadmap/` (`manifest.json`, `registry.yaml`, at least one JSON chunk under e.g. `roadmap/phases/`) so `specy-road validate` passes.
