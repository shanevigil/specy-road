...
- **Add** a dedicated subsection for specy-road **invariants**, for example:
  ...
  - **`abort-task-pickup`:** To release a claim after pickup without finishing, run `specy-road abort-task-pickup` on `feature/rm-<codename>` (clean tree). It deregisters on the integration branch with a forward commit and **push** — no `--no-push`. Never run it to tidy up **after** `finish-this-task`: it deletes the feature branch, which under `on_complete: pr` is the only place the finished work exists. See `docs/dev-workflow.md`.
  - **After `finish-this-task` with `on_complete: pr`:** the node is `Complete` and the claim released **on the feature branch**, while the integration branch shows neither until the PR merges — both are true at once. Leave the integration-branch registry row in place (it keeps the leaf off the available list) and do not re-pick the node. Pickup independently refuses a leaf whose `feature/rm-*` branch already marks it `Complete`, so a missing row is not a second chance. See `docs/git-workflow.md`.
  - **`grind-session` (autonomous / multi-leaf grinds):** For running many leaves or orchestrating sub-agents, use `specy-road grind-session` — it loops `do-next-available-task` → implement → `finish-this-task` using the **approved commands only** (it never hand-edits `roadmap/registry.yaml`). **Plan before parallelizing:** run `specy-road grind-session --plan --json` and dispatch only `parallel_batches[0]`; never start a later wave (or a `blocked`/`gated` node) until its predecessors are `Complete`. **Run it in a terminal, not in an IDE agent pane** — an unattended loop needs a process that can be waited on. When `--implement-cmd` is the Claude CLI, the loop waits out a timed session limit and resumes that session, so one limit does not end the run. See `docs/grind-session.md`.
...
- **Multi-leaf / sub-agent orchestration:** Use `specy-road grind-session` (approved-command loop; never hand-edit the registry). Run `specy-road grind-session --plan --json` first and only dispatch `parallel_batches[0]`; do not start a later wave or a `blocked`/`gated` node until predecessors are `Complete` (`docs/grind-session.md`).
