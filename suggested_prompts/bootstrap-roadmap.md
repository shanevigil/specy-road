...
- **`roadmap/activity.json`** — Optional sidecar recording when each node was last worked on (`node_key` → `{at, kind}`), written by the task-loop commands and shown as **Last worked** in the PM Gantt. Seed an existing repo with `specy-road backfill-activity`. Not part of `includes`.
